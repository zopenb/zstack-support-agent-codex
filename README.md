...
进入多来源查证、修复版本/回合确认、正式根因分析、来源冲突或深查路径时，如果需要 3 个及以上来源，事件分析默认尝试使用当前 Codex 宿主暴露的 subagent 调度工具并发查证，例如 `multi_agent_v1.spawn_agent`。没有 subagent 工具时会明确降级为主 agent 查证。
