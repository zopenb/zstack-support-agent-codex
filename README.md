...
如果需要同时录入连接器变量，使用隐藏输入模式：

```bash
./plugins/zstack-support/scripts/install-macos.sh --configure-env
```

也可以只打开可见 Terminal 配置窗口录入变量，不重复安装插件：

```bash
./plugins/zstack-support/scripts/open-env-config-window-macos.sh
```

macOS 脚本会使用 `launchctl setenv` 写入当前 GUI 用户会话，避免把 Token、密码、Authorization 或 base64 值落到仓库文件、shell profile 或日志中。
...
插件不保存任何账号、密码或 Token。每位同学都需要在自己的本机用户环境中配置：
...
2. 如果缺变量，发 `ZStackSupport:环境配置 帮我录入连接器变量`。Codex 会打开一个可见配置窗口，你在窗口里输入密钥；不要把密钥发到聊天里。
...
快照输出只包含变量名、是否存在、作用域、格式检查和修复建议。Windows 录入窗口会写入 Windows 用户变量，并尽量同步到当前进程；macOS 录入窗口会通过 `launchctl setenv` 写入当前 GUI 用户会话。MCP 工具注入通常仍需要重启 Codex 或新开线程。
...
Windows 推荐方式：从仓库根目录打开可见 PowerShell 配置窗口，一次录入四个变量。脚本只写入 Windows 用户变量，不打印密钥内容。
...
macOS 推荐方式：从仓库根目录打开可见 Terminal 配置窗口，一次录入四个变量。脚本使用隐藏输入，通过 `launchctl setenv` 写入当前 GUI 用户会话，不打印密钥内容。

```bash
./plugins/zstack-support/scripts/open-env-config-window-macos.sh
```

macOS 也可以直接在当前 Terminal 中执行安装和录入：
