...
只有用户明确要求“多 agent / 子 agent / subagent / 并行深查 / 多路并行深查”时，才使用本模板。普通 `@ZStackSupport:事件分析` 由主 agent 查证和汇总，不自行拉起并行 agent。
...
3. 内部跟踪 agent
   范围：只查 Jira/Confluence，输出编号、标题摘要、可点击链接、状态、影响版本、修复版本、版本边界。
   禁止：输出内部原文、评论原文、附件、账号、Token、Authorization 或原始 MCP 载荷。
...
命中：
链接：
要求：BBS 必须使用 `http://bbs.zstack.io/forum.php?mod=viewthread&tid=<tid>`；Jira/TIC/SUG/BUG 必须使用 `http://jira.zstack.io/browse/<KEY>`；不得输出相对链接或伪链接。
相关性：高 / 中 / 低
能支持的判断：
不能支持的判断：
证据边界：
