...
| 支持事件默认查证 | 用户粘贴客户反馈、日志、告警、失败 API、错误文本、截图转写、版本异常、兼容性问题、客户回复口径；即使没有明确说“分析”也属于支持事件 | 当前客户证据优先，先提取故障指纹并检索历史相同故障。优先查 BBS/Jira/Confluence 中的相同症状、已知缺陷、版本边界和处置路径；命中高度相同问题时给出处置路径和差异校验。未命中完全相同问题、相似度不足、来源冲突或涉及机制解释时，进入 GitHub 源码诊断。涉及源码、调用链、字段下发、版本合入、修复确认时，仍然必须查 GitHub。OS/厂商/外部生态加查 Tavily。输出可简洁，但必须说明证据边界 |
...
1. 证据优先：永远从当前客户证据开始，一二线支持场景优先检索历史相同故障；源码、ZStack知识社区和外部 Web 都是辅助参考，不能替代当前证据。
