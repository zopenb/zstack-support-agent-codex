...
| 支持事件默认查证 | 用户粘贴客户反馈、日志、告警、失败 API、错误文本、截图转写、版本异常、兼容性问题、客户回复口径；即使没有明确说“分析”也属于支持事件 | 当前客户证据优先，先提取故障指纹，并把 BBS、Jira、Confluence 作为内部知识系统整体查证：BBS 查历史相同/相似故障和处置路径，Jira 查已知缺陷/需求/修复状态，Confluence 查版本边界/标准口径/操作规范。只有内部知识系统查不到高度相同问题、相似度不足、来源冲突、内部结论无法解释当前证据，或涉及机制解释时，才进入 GitHub 源码诊断。涉及源码、调用链、字段下发、版本合入、修复确认时，仍然必须查 GitHub。OS/厂商/外部生态加查 Tavily。输出可简洁，但必须说明证据边界 |
| 轻量证据答复 | 用户问具体报错能否忽略、是否预期、兼容性边界、客户口径、是否影响功能；已有错误文本或截图可形成搜索词 | 简洁回答，但 ZStack 具体问题先查内部知识系统；内部知识不足或机制/源码/版本确认类必须查 GitHub |
...
- 先从当前客户证据提取故障指纹：错误文本、告警名、组件、版本、操作路径、资源类型、API、关键对象。
- 具体 ZStack 支持事件优先查询内部知识系统整体：BBS 查历史相同/相似故障和处置动作，Jira 查已知缺陷/需求和修复状态，Confluence 查版本边界、标准口径和操作规范。
- 内部知识系统命中高度相同问题时，给出处置路径、适用条件和差异校验；不要把“相似”写成“已确认相同”。
- BBS、Jira、Confluence 都查不到高度相同问题，或相似度不足、来源冲突、内部结论无法解释当前证据，或用户需要机制解释时，进入 GitHub 源码诊断。
- OS、内核、驱动、第三方生态：优先 Tavily 或厂商文档
- ZStack 具体事件的轻量答复必须先查内部知识系统；如果问题涉及源码、机制、调用链、类名、配置键、API、kvmagent、管理节点下发字段、代码定位、版本合入或修复确认，GitHub 必须查证。Jira/BBS/Confluence 只能补充历史、内部跟踪和边界，不能替代 GitHub 源码查证
...
```text
当前客户证据优先
→ 内部知识系统优先查证：BBS + Jira + Confluence
→ 内部知识命中高度相同问题时，给出处置路径和差异校验
→ BBS/Jira/Confluence 都查不到高度相同问题，或相似度不足/来源冲突/涉及机制解释时，再进入源码诊断
→ 涉及源码、调用链、字段下发、版本合入、修复确认时，仍然必须查 GitHub
```
...
- 产品版本、升级路径或外部依赖是否冲突
- 影响现象是否一致
- BBS 是否包含可复用处置动作或验证路径
- Jira 是否存在已知缺陷、需求、影响版本或修复状态
- Confluence 是否存在版本边界、标准口径、操作规范或兼容性说明

只有高度相同问题才能输出“可复用处置路径”。仅相似时输出“疑似同类问题”，并列出差异校验和下一步补证。

凡是问题包含以下任一特征，或内部知识系统查不到/解释不了当前证据，必须完成 GitHub 查证：

```text
源码 / 代码 / 调用链 / 机制 / 实现 / API / 类名 / 方法名 / 配置键 / 下发字段 / 版本合入 / 修复确认 /
kvmagent / management-server / KVMHost.java / vm_plugin.py / 报错堆栈 / NoneType / machineType / pciePortNums
```

执行要求：

- 若用户输入直接要求源码、调用链、字段下发、版本合入或修复确认，首个对应查证动作必须是 GitHub code/search/commit/tag/branch 或文件读取。
- 若主流程已经查完 BBS/Jira/Confluence，仍未命中高度相同问题、相似度不足、来源冲突或需要机制解释，必须切入 GitHub，不得继续扩大内部知识搜索来替代源码诊断。
...
- 如果主流程已经先查了 Jira/BBS/Confluence，发现问题实际是源码/机制/版本确认类，必须立即切回 GitHub。
...
1. 证据优先：永远从当前客户证据开始，一二线支持场景优先查证 BBS/Jira/Confluence 内部知识系统；源码、内部知识和外部 Web 都是辅助参考，不能替代当前证据。
