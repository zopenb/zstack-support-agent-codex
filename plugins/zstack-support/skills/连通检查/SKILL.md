...
第 3 步（认证变量）：检查是否存在 ATLASSIAN_AUTHORIZATION 环境变量；它应为完整 `Basic <base64(username:password)>`。如果只存在 ATLASSIAN_BASIC_AUTH，提示重新运行插件安装脚本派生运行时 Header。连通检查不得输出变量值
...
> Atlassian MCP 未配置。确认插件声明了 `zstack_atlassian_shared`，设置 `ATLASSIAN_BASIC_AUTH=base64(username:password)` 后重新运行插件安装脚本，生成 `ATLASSIAN_AUTHORIZATION=Basic <base64>`，再重启 Codex 或打开新线程。
