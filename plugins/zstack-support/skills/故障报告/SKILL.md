...
```powershell
python scripts/generate_fault_report.py --print-template
```

4. 将报告内容整理为 JSON，字段参考 [输入结构](references/input-schema.md)。
5. 使用脚本生成 DOCX：

```powershell
python scripts/generate_fault_report.py input.json --out output.docx
```
