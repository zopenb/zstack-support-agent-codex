...
| 支持事件默认查证 | 用户粘贴客户反馈、日志、告警、失败 API、错误文本、截图转写、版本异常、兼容性问题、客户回复口径；即使没有明确说“分析”也属于支持事件 | 当前客户证据优先，先提取故障指纹并检索历史相同故障。优先查 BBS/Jira/Confluence 中的相同症状、已知缺陷、版本边界和处置路径；命中高度相同问题时给出处置路径和差异校验。未命中完全相同问题、相似度不足、来源冲突或涉及机制解释时，进入 GitHub 源码诊断。涉及源码、调用链、字段下发、版本合入、修复确认时，仍然必须查 GitHub。OS/厂商/外部生态加查 Tavily。输出可以简洁，但必须说明证据边界 |
| 轻量证据答复 | 用户问具体报错能否忽略、是否预期、兼容性边界、客户口径、是否影响功能；已有错误文本或截图可形成搜索词 | 简洁回答，但 ZStack 具体问题至少查 2 个关键来源；一二线口径优先查历史相同故障，机制/源码/版本确认类必须查 GitHub |
...
- 具体 ZStack 支持事件优先检索历史相同故障：BBS 查相同症状和处置动作，Jira 查已知缺陷/需求和修复状态，Confluence 查版本边界和标准口径。
- 命中高度相同问题时，给出处置路径、适用条件和差异校验；不要把“相似”写成“已确认相同”。
- 未命中完全相同问题、相似度不足、来源冲突、历史处置无法解释当前证据，或用户需要机制解释时，进入 GitHub 源码诊断。
- OS、内核、驱动、第三方生态：优先 Tavily 或厂商文档
- ZStack 具体事件的轻量答复也必须至少查 2 个关键来源；如果问题涉及源码、机制、调用链、类名、配置键、API、kvmagent、管理节点下发字段、代码定位、版本合入或修复确认，GitHub 必须查证。Jira/BBS/Confluence 只能补充历史、内部跟踪和边界，不能替代 GitHub 源码查证
...
- 产品版本、升级路径或外部依赖是否冲突
- 影响现象是否一致
- 历史案例是否包含可复用处置动作或验证路径

只有高度相同问题才能输出“可复用处置路径”。仅相似时输出“疑似同类问题”，并列出差异校验和下一步补证。

凡是问题包含以下任一特征，或历史相同故障无法解释当前证据，必须完成 GitHub 查证：
...
- 若主流程先查历史相同故障，发现未命中完全相同问题、相似度不足、来源冲突或需要机制解释，必须切入 GitHub，不得继续扩大 Jira/Confluence/BBS 搜索来替代源码诊断。
...
期望：先提取故障指纹并查 BBS/Jira/Confluence 的相同故障或已知问题；命中时输出处置路径和差异校验；未命中或相似度不足时进入 GitHub 机制查证。
