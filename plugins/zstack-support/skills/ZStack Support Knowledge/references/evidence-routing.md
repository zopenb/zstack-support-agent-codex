...
本规则用于决定是否回答、追问、单点查证、多方查证或深查。所有技能优先引用本文件，避免在多个技能中重复维护路由逻辑。
...
| 轻量证据答复 | 用户问具体报错能否忽略、是否预期、兼容性边界、客户口径、是否影响功能；已有错误文本或截图可形成搜索词 | 查询 1-2 个最相关来源后简洁回答 |
...
完整事件分析只在用户明确要求分析事件、故障、根因、交接或闭环时启动。简单概念问题不要默认跑完整工作流；但具体报错、兼容性、能否忽略、客户回复口径不能纯直答，必须进入轻量证据答复或更高等级查证。
...
- 兼容性、已知问题、客户口径：优先 BBS、官网文档、Jira 或 Confluence
- OS、内核、驱动、第三方生态：优先 Tavily 或厂商文档
- 通常查询 1-2 个最相关来源；只有证据冲突或风险高时再扩展
...
| 来源 | 适用问题 | 不适用边界 |
|------|----------|------------|
| GitHub | API、类名、配置键、错误文本、调用链、源码机制、版本差异 | 不能证明客户环境实际行为 |
...
`agents/openai.yaml` 只声明 UI 元数据和 MCP 依赖，不会自动创建或启动 subagent。Subagent 不是插件静态配置能力，必须由当前会话暴露 subagent 调度工具，并由主 agent 在满足触发条件时显式派发。

默认使用主 agent 深查，不默认启用多 agent。

| 深查方向 | 触发条件 | 输出目标 |
|----------|----------|----------|
| 主 agent 深查 | 任一深查触发条件成立，但用户未明确要求多 agent，或当前会话未暴露 subagent 工具 | 当前主 agent 完成查证、合并和边界说明 |
| 源码深查 subagent | 用户明确要求多 agent / 子 agent / 并行深查，且 GitHub 搜到入口但调用链未追完；版本差异需要确认；源码与现象存在缺口 | GitHub commit、tag、release branch、调用链、关键分支、版本差异、机制边界 |
| 历史案例 subagent | 用户明确要求多 agent / 子 agent / 并行深查，且 BBS 命中过多；需要筛选相似度；历史案例之间结论冲突 | BBS 相似案例排序、差异、可复用验证动作 |

Tavily 和官网文档暂不 agent 化，保持工具查询即可。

### Subagent 显式调度规则

只有同时满足以下条件才触发 subagent：

- 用户明确要求“多 agent / 子 agent / subagent / 并行深查 / 多路并行深查”。
- 当前 Codex 会话可发现并调用 subagent 调度工具。
- 任务可拆成互不覆盖的有界子任务。

当用户明确要求多 agent 时，主 agent 必须先确认是否有 subagent 调度工具。若宿主提供工具发现能力，先搜索 `subagent`、`multi agent` 或 `spawn_agent`；若当前工具列表已明确存在可调用的 subagent 调度工具，可直接使用。没有找到或无法调用时，标注失败层为“工具未暴露”，并降级为主 agent 查证。

如果用户未明确要求多 agent，不做 subagent 工具发现。

触发后：

- 主 agent 保持总控，继续整理问题、关键路径查证和最终合并。
- 源码深查 subagent 只查 GitHub，不查 Jira/Confluence。
...
```text
Subagent 状态：未触发
原因：用户未明确要求多 agent / 当前会话未暴露 subagent 调度工具 / 任务不适合拆分
...
失败层标注：
路由失败 / GitHub 未查 / subagent 未触发 / 工具未暴露 / 结论越界
```
