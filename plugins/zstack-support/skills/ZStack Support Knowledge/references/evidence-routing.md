...
| 来源 | 适用问题 | 不适用边界 |
|------|----------|------------|
| GitHub | API、类名、配置键、错误文本、调用链、源码机制、版本差异、版本合入、修复确认 | 不能证明客户环境实际行为；但源码/机制/版本类结论必须有 GitHub 查证 |
| ZStack知识社区(BBS) | 历史相同或相似案例、相同症状、可复用验证动作和处置路径 | 不能单独作为当前事件根因；仅相似时必须输出差异校验 |
...
内部知识优先复测：
输入：客户 4.8.0 备份服务器告警“镜像存储可用容量百分比<10%”，但现场实际是 ZStack-backup 空间问题，怎么回复？
期望：先提取故障指纹，并把 BBS、Jira、Confluence 作为内部知识系统整体查证；内部知识命中高度相同问题时输出处置路径和差异校验；三者都未命中或相似度不足时进入 GitHub 机制查证。
