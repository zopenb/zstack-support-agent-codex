...
ZStack Support Agent（支持分析），基于证据优先方法论，结合 ZStack知识社区(BBS) 历史参考、Jira/Confluence 内部只读参考、GitHub 源码分析（zstack + zstack-utility）和 Tavily 外部 Web/厂商论坛参考，按问题类型动态选择直接回答、主动支持事件查证、单点查证、多来源查证或完整事件分析。具体 ZStack 客户事件采用“当前客户证据优先 → BBS/Jira/Confluence 内部知识系统整体查证 → 命中高度相同问题时给出处置路径和差异校验 → 内部知识系统都查不到高度相同问题、相似度不足、来源冲突或涉及机制解释时进入源码诊断”的流程；涉及源码、调用链、字段下发、版本合入、修复确认时，仍然必须查 GitHub。需要并行时请显式要求“用多 agent 并行深查”。内置 E0-E5 证据分级体系、4 类中文公开标签和 11 种闭环决策类型。
...
| 命令 | 说明 |
|------|------|
| `@ZStackSupport:事件分析` | 粘贴事件描述/错误日志/截图，自动完成证据分析、内部知识系统查证、必要源码查证和闭环决策 |
...
| 具体支持事件 | 客户升级后云主机迁移失败，日志如下... | 当前客户证据优先，先把 BBS/Jira/Confluence 作为内部知识系统整体查证；三者都查不到高度相同问题、相似度不足、来源冲突或涉及机制解释时进入源码诊断；涉及源码、调用链、字段下发、版本合入、修复确认时必须查 GitHub；需要并行时请显式要求“用多 agent 并行深查” |
...
| 技能 | 说明 |
|------|------|
| 事件分析 | 核心技能。先判断入口类型；低风险概念问题直接答，具体支持事件先提取故障指纹，并把 BBS/Jira/Confluence 作为内部知识系统整体查证；内部知识命中高度相同问题时输出处置路径和差异校验；三者都未命中、相似度不足、来源冲突或涉及机制解释时进入 GitHub 源码诊断；复杂事件输出 Intake、证据映射、参考证据、安全行动建议和闭环决策 |
