...
ZStack Support Agent（源码级分析），基于证据优先方法论，结合 GitHub 源码分析（zstack + zstack-utility）、ZStack知识社区(BBS) 历史参考、Tavily 外部 Web/厂商论坛参考和 Jira/Confluence 内部只读参考，按问题类型动态选择直接回答、轻量证据答复、单点查证、多来源查证或完整事件分析。内置 E0-E5 证据分级体系、4 类中文公开标签和 11 种闭环决策类型。
...
| 轻量证据答复 | 这个报错能忽略吗？怎么回复客户？ | 查 1-2 个关键来源后简洁回答 |
...
| 技能 | 说明 |
|------|------|
| 事件分析 | 核心技能。先判断入口类型，再按需选择直接回答、轻量证据答复、最小追问、单点查证、多来源并行查证或完整事件分析。复杂事件输出 Intake、证据映射、参考证据、安全行动建议和闭环决策 |
