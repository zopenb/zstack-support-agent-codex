...
| 具体支持事件 | 客户升级后云主机迁移失败，日志如下... | 默认主动查 GitHub + BBS + Jira；涉及版本边界/标准口径时加查 Confluence；涉及 OS/厂商问题时加查 Tavily；3 个及以上来源默认尝试 subagent 并发 |
...
| 技能 | 说明 |
|------|------|
| 事件分析 | 核心技能。先判断入口类型；低风险概念问题直接答，具体 ZStack 支持事件默认主动查 GitHub+BBS+Jira，涉及版本边界/标准口径时补查 Confluence；3 个及以上来源默认尝试 subagent 并发；复杂事件输出 Intake、证据映射、参考证据、安全行动建议和闭环决策 |

## 默认并发查证

进入多来源查证、修复版本/回合确认、正式根因分析、来源冲突或深查路径时，如果需要 3 个及以上来源，事件分析默认尝试使用当前 Codex 宿主暴露的 subagent 调度工具并发查证，例如 `multi_agent_v1.spawn_agent`。如果当前会话没有 subagent 工具，则明确降级为主 agent 查证。
