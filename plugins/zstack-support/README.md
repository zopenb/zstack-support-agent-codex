...
| 具体支持事件 | 客户升级后云主机迁移失败，日志如下... | 先按问题性质路由：源码/机制类必须先查 GitHub；历史相似查 BBS；已知缺陷/版本跟踪查 Jira；版本边界/标准口径查 Confluence；OS/厂商问题查 Tavily；3 个及以上来源默认尝试 subagent 并发 |
...
| 技能 | 说明 |
|------|------|
| 事件分析 | 核心技能。先判断入口类型；低风险概念问题直接答，源码/机制类事件必须先查 GitHub，历史相似查 BBS，已知缺陷/版本跟踪查 Jira，版本边界/标准口径查 Confluence；3 个及以上来源默认尝试 subagent 并发；复杂事件输出 Intake、证据映射、参考证据、安全行动建议和闭环决策 |
